...
async def validate_gemini_models():
    # Return whitelist directly to allow fallback logic in ask_gemini to handle availability
    return GEMINI_MODEL_WHITELIST
...
async def ask_gemini(model: str, messages: list):
    ...
    # ✅ Gemini needs ONE clean prompt (NO roles)
    prompt_parts = []
    for m in messages:
        if m["role"] == "system":
    ...
    try:
        def call():
            instance = genai.GenerativeModel(full_model)
            return instance.generate_content(final_prompt)
        ...
        if model != DEFAULT_GEMINI_FALLBACK:
            try:
                fallback = normalize_gemini_model(DEFAULT_GEMINI_FALLBACK)

                def fb():
                    instance = genai.GenerativeModel(fallback)
                    return instance.generate_content(final_prompt)
                ...
                return f"Gemini fatal error: {fe}"
